...
const RegistrationRequestSchema = new Schema({
  // --- meta (matches your ER) ---
  status: { type: String, enum: ['pending', 'approved', 'rejected'], default: 'pending' },
  reviewedBy_userID: { type: Types.ObjectId, ref: 'Web_Owner' },
  reviewedAt: Date,
  //submittedBy_userID: { type: Types.ObjectId, ref: 'Admin' },
  submittedAt: { type: Date, default: Date.now },

  // when you implement approval later you can fill these:
  ...
  application: {
    ...
    admin: {
      ...
      password:   { type: String, required: true } // (plaintext snapshot only for now; you’ll hash on approval later)
    }
  }
