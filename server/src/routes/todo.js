...
router.get('/:traineeId', async (req, res) => {
  try {
    ...
    const items = await Todo.find({ traineeId }).sort({ createdAt: -1 });
    res.json(items);
  } catch (err) {
...
router.post('/', async (req, res) => {
  try {
    ...
    const created = await Todo.create({ traineeId, day, title });
...
router.delete('/completed/:traineeId', async (req, res) => {
  try {
    const { traineeId } = req.params;
    const result = await Todo.deleteMany({ traineeId, isCompleted: true });
