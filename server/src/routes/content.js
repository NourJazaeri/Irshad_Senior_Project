...
import { requireAdmin, requireAdminOrSupervisor, requireTrainee } from '../middleware/authMiddleware.js';
...
async function createInitialProgressRecords(content) {
  try {
    ...
    if (content.assignedTo_traineeID && Array.isArray(content.assignedTo_traineeID) && content.assignedTo_traineeID.length > 0) {
      // Assigned to specific trainee(s) - array with actual trainees
    ...
    } else if (content.assignedTo_traineeID && !Array.isArray(content.assignedTo_traineeID)) {
      // Single trainee (not array)
      ...
      const trainees = await Trainee.find({ ObjectGroupID: content.assignedTo_GroupID });
      if (trainees && trainees.length > 0) {
        const groupTrainees = trainees.map(trainee => trainee._id);
        console.log('✅ Found', groupTrainees.length, 'trainees in group');
        traineesToAssign.push(...groupTrainees);
      } else {
        console.log('⚠️ No trainees found in group');
      }
 ...
 */
router.post('/youtube', requireAdminOrSupervisor, upload.none(), async (req, res) => {
  try {
    ...
    // Only set assignedTo_traineeID if there are specific trainees
    if (traineeIds.length > 0) {
      contentData.assignedTo_traineeID = traineeIds;
    }
 ...
 */
router.post('/link', requireAdminOrSupervisor, upload.none(), async (req, res) => {
  try {
