...
router.get("/dashboard/:supervisorId", async (req, res) => {
  try {
    ...
    console.log('DEBUG: Progress found for supervisor', supervisorId, progress);

    // 1. Group Progress Overview - Calculate based on ALL assigned content
    const groupStats = await Promise.all(groups.map(async group => {
      ...
      const groupProgress = progress.filter(p => p.groupID && p.groupID.toString() === group._id.toString());
