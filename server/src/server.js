...
import groupsRouter from './routes/groups.js'; // ✅ مضاف حديثًا - merged with groupRoutes
import employeesRouter from './routes/employees.js'; // ✅ مضاف حديثًا
...
app.use('/api/groups', groupsRouter); // ✅ إضافة groups.js - merged with groupRoutes
app.use('/api/employees', employeesRouter); // ✅ إضافة الموظفين فعليًا
  ...
  app.get('/', (req, res) => {
    res.json({
      ...
      endpoints: {
        ...
        groups: '/api/groups/by-department/:departmentId',
        finalizeGroup: '/api/groups/finalize',
        editGroup: '/api/groups/:id (PUT)',
        deleteGroup: '/api/groups/:id (DELETE)',
        debugTrainees: '/api/groups/debug-trainees (GET)',
        cleanupOrphanedTrainees: '/api/groups/cleanup-orphaned-trainees (POST)',
        forceCleanupAllTrainees: '/api/groups/force-cleanup-all-trainees (POST)',
        testEmail: '/api/groups/test-email (POST)',
        employees: '/api/employees/by-department?departmentName=XYZ',
      },
    });
  });
}
...
(async () => {
  try {
    ...
    app.listen(PORT, () => {
      ...
      console.log('   GET  /api/groups/debug-trainees ✅');
      console.log('   POST /api/groups/cleanup-orphaned-trainees ✅');
      console.log('   POST /api/groups/force-cleanup-all-trainees ✅');
      console.log('   POST /api/groups/test-email ✅');
      console.log('   CRUD /api/employees ✅');
    });
  } catch (error) {
