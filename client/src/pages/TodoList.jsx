...
export default function TodoList() {
  // Get traineeId from localStorage - parse the user object
  const getUserId = () => {
    try {
  ...
  const loadTodos = async () => {
    if (!traineeId) {
      console.error("No traineeId found. User may not be logged in.");
      setLoading(false);
      return;
    }
  ...
  const handleAddFromTaskInput = async (day, title) => {
    ...
    if (!traineeId) {
      console.error("Cannot add todo: traineeId is null");
      alert("Error: User not logged in. Please refresh the page and log in again.");
      return;
    }
